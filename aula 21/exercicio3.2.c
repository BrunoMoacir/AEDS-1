...
void menorAprovado(int turma[], int n){
    int menorNotaAprovado = 100;
    for(int i = 0; i < n; i++){
        if(turma[i] > 60 && turma[i] < menorNotaAprovado){
            menorNotaAprovado = turma [i];
        }
    }
    printf("\n A menor nota aprovada foi %d", menorNotaAprovado);
}